$(document).ready(function ($) {
    ...
    $('a[name="exportBtn"]').on("click", function () {
        ...
        $('#atome_trans_backup_table').DataTable().$('tr', { "filter": "applied" }).each(function () {
        ...
        if (checkboxValues.length > 0) {
            ...
            window.location.href = "atome_trans_backup_table.php";
        } else {
    ...
    function updateCheckboxesOnOtherPages(isChecked) {
        ...
        var cells = $('#atome_trans_backup_table').DataTable().cells().nodes();
