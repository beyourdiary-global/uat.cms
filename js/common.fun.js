...
function datatableAlignment(elementID)
{
	$(window).on('load resize', () => {
		...
		if(!(tableParentElement.hasClass('p-0')))
			tableParentElement.addClass('p-0');

		// info
		if(!(infoElement.hasClass('mb-3')))
			infoElement.addClass('mb-3 pb-3');
...
async function confirmationDialog(id, msg, pagename, path, pathreturn, act) {
    switch(act)
    {
		...
		case 'PC':  var title = "Successful Change " + pagename;
					break;
		case 'F':	var title = "An error occurred. Please try again later.";
					break;
	...
	if(act == 'I' || act == 'E' || act == 'NC' || act == 'PC' || act == 'F')
	{
		const myModal2 = new bootstrap.Modal(modelResult, {
			keyboard: false,
			backdrop: 'static'
