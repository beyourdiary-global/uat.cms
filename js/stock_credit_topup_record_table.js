...
$(document).ready(function ($) {
    ...
    $('a[name="exportBtn"]').on("click", function () {
        ...
        $('#stock_credit_table').DataTable().$('tr', { "filter": "applied" }).each(function () {
        ...
        if (checkboxValues.length > 0) {
            ...
            window.location.href = "stock_credit_top_up_request_table.php";
        } else {
    ...
    function updateCheckboxesOnOtherPages(isChecked) {
        ...
        var cells = $('#stock_credit_table').DataTable().cells().nodes();
