...
$(document).ready(function ($) {
    ...
    $('a[name="exportBtn"]').on("click", function () {
        ...
        $('#merchant_comm_table').DataTable().$('tr', { "filter": "applied" }).each(function () {
        ...
        if (checkboxValues.length > 0) {
            ...
            window.location.href = "merchant_comm_record_table.php";
        } else {
    ...
    function updateCheckboxesOnOtherPages(isChecked) {
        ...
        var cells = $('#merchant_comm_table').DataTable().cells().nodes();
