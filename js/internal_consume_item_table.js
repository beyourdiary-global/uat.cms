...
$(document).ready(function ($) {
    ...
    $('a[name="exportBtn"]').on("click", function () {
        ...
        $('#internal_consume_item_table').DataTable().$('tr', { "filter": "applied" }).each(function () {
        ...
        if (checkboxValues.length > 0) {
            ...
            window.location.href = "internal_consume_item_table.php";
        } else {
    ...
    function updateCheckboxesOnOtherPages(isChecked) {
        ...
        var cells = $('#internal_consume_item_table').DataTable().cells().nodes();
