        ...
        case 'addPltf': case 'updPltf':
            ...
            if($pltf_name)
            {
                if($action == 'addPltf')
                {
                    try
                    {
                        $query = "INSERT INTO ".PLTF."(name,remark,create_by) VALUES ('$pltf_name','$pltf_remark','".$_SESSION['userid']."')";
                        ...
                        $log['act_msg'] = $_SESSION['user_name'] . " added [id=$last_id] $pltf_name into Platform Table.";
                        $log['query_rec'] = $query;
                ...
                else
                {
                    try
                    {
                        ...
                        $log['ctime'] = $ctime;
                        $log['uid'] = $log['cby'] = $_SESSION['userid'];
                        $log['act_msg'] = $_SESSION['user_name'] . " edited the data [id=$pltf_id] $pltf_name from Platform Table.";
                        $log['query_rec'] = $query;
                        $log['query_table'] = PLTF;
                        $log['page'] = 'Platform';
                        $log['oldval'] = $oldval;
            ...
            break;
        case 'back':
            header('Location: platform_table.php');
            break;
    }
}
...
if(post('act') == 'D')
{
    ...
    if($id)
    {
        try
        {
            ...
            $log['ctime'] = $ctime;
            $log['uid'] = $log['cby'] = $_SESSION['userid'];
            $log['act_msg'] = $_SESSION['user_name'] . " deleted the data [id=$pltf_id] $pltf_name from Platform Table.";
            $log['query_rec'] = $query;
            $log['query_table'] = PLTF;
            $log['page'] = 'Platform';
            $log['connect'] = $connect;
...
if(($pltf_id != '') && ($act == '') && (isset($_SESSION['userid'])) && ($_SESSION['viewChk'] != 1) && ($_SESSION['delChk'] != 1))
{
    ...
    $log['ctime'] = $ctime;
    $log['uid'] = $log['cby'] = $_SESSION['userid'];
    $log['act_msg'] = $_SESSION['user_name'] . " viewed the data [id=$pltf_id] $pltf_name from Platform Table.";
    $log['page'] = 'Platform';
    $log['connect'] = $connect;
    audit_log($log);
}
