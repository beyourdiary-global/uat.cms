...
if (post('actionBtn')) {
    ...
    switch ($action) {
        ...
        case 'updData':
            ...
            if (isDuplicateRecord("date", $holidayDate, $tblName, $connect, $dataID)) {
                $err = "Duplicate record found for " . $pageTitle . " name.";
                break;
            }
            ...
            if ($action == 'addData') {
                ...
                try {
                    ...
                    if ($oldvalarr && $chgvalarr) {
                        $query = "UPDATE " . $tblName . " SET name ='$currentDataName', date ='$holidayDate', update_date = curdate(), update_time = curtime(), update_by ='" . USER_ID . "' WHERE id = '$dataID'";
                        $returnData = mysqli_query($connect, $query);
                    } else {
                        $act = 'NC';
                    }
