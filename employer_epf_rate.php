...
$employer_epf_rate_id = input('id');
$act = input('act');
$redirect_page = $SITEURL . '/employer_epf_rate_table.php';
...
if(post('actionBtn'))
{
    ...
    switch($action)
    {
        case 'addemployer_rate_epf': case 'updemployer_rate_epf':
...
if(post('act') == 'D')
{
    ...
    if($id)
    {
        try
        {
            ...
            $employer_epf_rate = $row['epf_rate'];

            $query = "DELETE FROM ".EMPLOYER_EPF." WHERE id = ".$id;
            mysqli_query($connect, $query);
            ...
            $log['ctime'] = $ctime;
            $log['uid'] = $log['cby'] = USER_ID;
            $log['act_msg'] = USER_NAME . " deleted the data <b>$employer_epf_rate</b> from <b><i>$pageTitle Table</i></b>.";
            $log['query_rec'] = $query;
            $log['query_table'] = EMPLOYER_EPF;
            $log['page'] = $pageTitle ;
            $log['connect'] = $connect;
...
if(($employer_epf_rate_id != '') && ($act == '') && (USER_ID != '') && ($_SESSION['viewChk'] != 1) && ($_SESSION['delChk'] != 1))
{
    $employer_epf_rate = isset($dataExisted) ? $row['epf_rate'] : '';
    $_SESSION['viewChk'] = 1;
    ...
    $log['ctime'] = $ctime;
    $log['uid'] = $log['cby'] = USER_ID;
    $log['act_msg'] = USER_NAME . " viewed the data <b>$employer_epf_rate</b> from <b><i>$pageTitle Table</i></b>.";
    $log['page'] = $pageTitle;
    $log['connect'] = $connect;
    audit_log($log);
}
...
<div id="employerStatusFromContainer" class="container d-flex justify-content-center">
    <div class="col-6 col-md-6 formWidthAdjust">
        <form id="employerStatusFrom" method="post" action="">
            ...
            <?php
                ...
                {
                    case 'I':
                        echo '<button class="btn btn-lg btn-rounded btn-primary mx-2 mb-2" name="actionBtn" id="actionBtn" value="addemployer_rate_epf">Add '.$pageTitle.' </button>';
                        break;
                    case 'E':
                        echo '<button class="btn btn-lg btn-rounded btn-primary mx-2 mb-2" name="actionBtn" id="actionBtn" value="updemployer_rate_epf">Edit '.$pageTitle.' </button>';
                        break;
                }
            ?>
...
*/
centerAlignment("employerStatusFromContainer");
</script>
</body>
</html>