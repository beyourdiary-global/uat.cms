...
if(post('actionBtn'))
{
    ...
    switch($action)
    {
        case 'addWarehouse': case 'updWarehouse':
            ...
            if (!$warehouse_name){
                $err = "Warehouse name cannot be empty.";
                break;
            }
            else if(isDuplicateRecord("name", $warehouse_name, WHSE, $connect, $warehouse_id)){
                $err = "Duplicate record found for warehouse name.";
                break;
            }
            else if($action == 'addWarehouse'){
                try
                {
