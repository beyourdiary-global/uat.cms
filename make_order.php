...
if(post('actionBtn'))
{
    ...
    switch($act)
    {
        case 'makeOrder':
            ...
            $dataMO['content'] = postEmptyFilter('parcel_content');
            $dataMO['value'] = postEmptyFilter('parcel_value');
            $dataMO['sid'] = $sid;
            $dataMO['pick_point'] = post('sender_dropoffpoint') ? post('sender_dropoffpoint') : '';
            $dataMO['pick_name'] = postEmptyFilter('sender_name');
            $dataMO['pick_company'] = post('sender_company');
            $dataMO['pick_contact'] = post('sender_tel_code') . postEmptyFilter('sender_tel');
            $dataMO['pick_mobile'] = post('sender_alt_tel_code') . postEmptyFilter('sender_alt_tel');
            $dataMO['pick_addr1'] = postEmptyFilter('sender_addr_1');
            $dataMO['pick_addr2'] = postEmptyFilter('sender_addr_2');
            $dataMO['pick_city'] = postEmptyFilter('sender_city');
            $dataMO['pick_state'] = $area_from;
            $dataMO['pick_code'] = post('sender_postcode');
            $dataMO['pick_country'] = $from;
            $dataMO['send_point'] = '';
            $dataMO['send_name'] = postEmptyFilter('receiver_name');
            $dataMO['send_company'] = post('receiver_company');
            $dataMO['send_contact'] = post('receiver_tel_code') . postEmptyFilter('receiver_tel');
            $dataMO['send_mobile'] = post('receiver_alt_tel_code') . postEmptyFilter('receiver_alt_tel');
            $dataMO['send_addr1'] = postEmptyFilter('receiver_addr_1');
            $dataMO['send_addr2'] = postEmptyFilter('receiver_addr_2');
            $dataMO['send_city'] = postEmptyFilter('receiver_city');
            $dataMO['send_state'] = $area_to;
            $dataMO['send_code'] = post('receiver_postcode');
            $dataMO['send_country'] = $to;
            $dataMO['collect_date'] = $pickup_date;
            $dataMO['send_email'] = postEmptyFilter('receiver_email');
            $dataMO['reference'] = postEmptyFilter('parcel_reference');
