...
//Delete Data
if ($act == 'D') {
    deleteRecord($tblName, $dataID, $row['unit'], $connect, $connect, $cdate, $ctime, $pageTitle);
    $_SESSION['delChk'] = 1;
}
...
if ($dataID && !$act && USER_ID && !$_SESSION['viewChk'] && !$_SESSION['delChk']) {
    ...
    if (isset($errorExist)) {
        ...
        $viewActMsg = USER_NAME . " viewed the data <b>" . $row['unit'] . "</b> from <b><i>$tblName Table</i></b>.";
    }
...
if (post('actionBtn')) {
    ...
    switch ($action) {
        ...
        case 'updData':
            ...
            $currentDataUnit = postSpaceFilter('currentDataUnit');
            ...
            if (isDuplicateRecord("unit", $currentDataUnit, $tblName, $connect, $dataID)) {
                $err = "Duplicate record found for " . $pageTitle . " unit.";
                break;
            }
            ...
            if ($action == 'addData') {
                try {
                    ...
                    if ($currentDataUnit)
                        array_push($newvalarr, $currentDataUnit);

                    if ($dataRemark)
                        array_push($newvalarr, $dataRemark);

                    $query = "INSERT INTO " . $tblName . "(unit,remark,create_by,create_date,create_time) VALUES ('$currentDataUnit','$dataRemark','" . USER_ID . "',curdate(),curtime())";
                ...
                try {
                    if ($row['unit'] != $currentDataUnit) {
                        array_push($oldvalarr, $row['unit']);
                        array_push($chgvalarr, $currentDataUnit);
                    }
                    ...
                    if ($oldvalarr && $chgvalarr) {
                        $query = "UPDATE " . $tblName . " SET unit ='$currentDataUnit', remark ='$dataRemark', update_date = curdate(), update_time = curtime(), update_by ='" . USER_ID . "' WHERE id = '$dataID'";
                        $returnData = mysqli_query($connect, $query);
                    } else {
                        $act = 'NC';
                    }
            ...
            if (isset($query)) {
                ...
                if ($pageAction == 'Add') {
                    ...
                    if (isset($returnData)) {
                        $log['act_msg'] = USER_NAME . " added <b>$currentDataUnit</b> into <b><i>$tblName Table</i></b>.";
                    } else {
                        $log['act_msg'] = USER_NAME . " fail to insert <b>$currentDataUnit</b> into <b><i>$tblName Table</i></b> ( $errorMsg )";
                    }
