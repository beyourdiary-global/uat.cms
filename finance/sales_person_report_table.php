                ...
                <div class="row mb-3">
                    ...
                    <label class="form-label">Group by:</label>
                        <select class="form-select" id="group">
                            <option value="person"selected>Sales Person In Charge</option>
                            <option value="platform" >Platform</option>
                        </select>
                    </div>
                    <div class="col-md-2 d-flex align-items-center justify-content-center">
                    <a id='resetButton' href="../reset.php?redirect=/finance/sales_person_report_table.php" class="btn btn-sm btn-rounded btn-primary"> <i class="fa fa-refresh"></i> Reset </a>
                    </div>
                </div>
        <table class="table table-striped" id="sales_person_report_table">
            <thead>
                <tr>
                  ...
                  if ($result4) {
                     ...
                     foreach ($resultSets as $resultSetKey => $resultSet) {
                        while ($row = $resultSet->fetch_assoc()) {
                            ...
                            if (isset($row['sales_pic']) || isset($row['pic'] )) {
                                $pic = isset($row['sales_pic']) ? $row['sales_pic'] : $row['pic'];
                                $q1 = getData('name', "id='" . $pic . "'", '', USR_USER, $connect);
                                $pic_fetch = $q1->fetch_assoc();
                                $pic_name = isset($pic_fetch['name']) ? $pic_fetch['name'] : '';
                            } 
                            ...
                            if ($groupOption && $groupOption3) {
                                ...
                                else if (($groupOption === 'person' || $groupOption === 'platform' ) && $groupOption4 !== 'daily') {
                            ...
                            foreach ($groupedRows as $key => $groupedRow) {
                                if (isset($key)) {
                                    ...
                                    audit_log($log);
                                $ids = implode(',', $groupedRow['ids']);
                                $url = "sales_person_report_table_summary.php?ids=" . urlencode($ids) . "&key=" . urlencode($key);
