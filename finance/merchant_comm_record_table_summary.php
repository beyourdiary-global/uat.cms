...
$img_path = '../' . img_server . 'finance/merchant_comm_record/';
...
if (!empty($checkboxValues)) {
    ...
    $excelData = array(
        array('S/N', 'MERCHANT ID', 'DATE', 'CURRENCY UNIT', 'AMOUNT','REMARK','CREATE BY', 'CREATE DATE', 'CREATE TIME', 'UPDATE BY', 'UPDATE DATE', 'UPDATE TIME')
    );    // Get the data from the database using the WHERE clause
    $query2 = $finance_connect->query("SELECT * FROM " . MRCHT_COMM . " WHERE status = 'A' AND id IN ($checkboxValues) ORDER BY merchantID ASC, date ASC, currency ASC, amount ASC");
    ...
    if ($query2->num_rows > 0) {
        while ($row2 = $query2->fetch_assoc()) {
            ...
            $columnNames = array('merchantID', 'date', 'currency', 'amount','remark','create_by', 'create_date', 'create_time', 'update_by', 'update_date', 'update_time');
...
<?php include "../js/merchant_comm_table.js" ?>
    /**
  oufei 20231014
  common.fun.js
  function(void)
