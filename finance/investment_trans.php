            ...
            if (($ivs_mrcht == 'other') && !isset($mrcht_other)) {
                ...
                try {
                    ...
                    if ($row['remarks'] != $ivs_remark) {
                        array_push($oldvalarr, $row['remarks'] == '' ? 'Empty Value' : $row['remarks']);
                        array_push($chgvalarr, $ivs_remark == '' ? 'Empty Value' : $ivs_remark);
                        array_push($datafield, 'remarks');
                    }
...
if (($row_id) && !($act) && (USER_ID != '') && ($_SESSION['viewChk'] != 1) && ($_SESSION['delChk'] != 1)) {
    ...
    if (isset($errorExist)) {
        ...
        $viewActMsg = USER_NAME . " viewed the data [<b> ID = " . $row_id . "</b> ] <b>" . $trans_id . "</b> from <b><i>$tblName Table</i></b>.";
    }
